...
version: 1.0.0
description: >
  Batch-score scraped jobs into a ranked shortlist using the candidate profile and fit framework. Trigger when the user asks to rank, triage, or shortlist saved job postings.
context: fork
---

# rank workflow - Triage Scraped Jobs into a Ranked Shortlist

You are batch-scoring the jobs that `the job-scraper skill` has collected, so the user can decide where to spend `the apply skill` effort. `the job-scraper skill` finds and dedupes postings; `the apply skill` evaluates one at a time in depth. `the rank skill` is the bridge: it scores every new posting against the fit framework and returns a ranked shortlist.

`the rank skill` produces **triage scores**, not final evaluations. It scores from the posting text and the candidate profile only - no company research, no reviewer subagent. `the apply skill`'s Step 1 evaluation (which adds company research) remains authoritative and always re-runs when the user applies.
...
- A focus area (e.g. `the rank skill data science`) -> rank only jobs whose title or stored fit-notes match the focus
...
1. Read `job_scraper/seen_jobs.json`. If the file is missing or has no entries, tell the user to run `the job-scraper skill` first and stop.
...
Each subagent returns a JSON array, one object per job:
...
Do not modify `job_search_tracker.csv` - that file records applications, and `the rank skill` never applies. Re-running `the rank skill` is idempotent: already-`ranked` jobs are skipped unless `--all` re-scores them.
...
- Say explicitly that these are **triage scores from the posting text only**, and that `the apply skill` will re-evaluate with company research before anything is drafted.
- Then ask: "Want to apply to any of these? Give me the number(s) and I'll start with the full `the apply skill` workflow."
- If the user picks one, run the `the apply skill` workflow on that job's URL, passing the triage verdict as prior context but **re-running the full Step 1 evaluation** - triage never substitutes for it.
...
2. **Triage depth only.** No company research, no salary lookups, no reviewer subagents - `the rank skill` exists to be cheap enough to run on every scrape batch.
...
5. **State stays consistent.** `seen_jobs.json` fields are only added, never restructured, so `the job-scraper skill`'s dedup keeps working; the tracker is read-only for this command.
