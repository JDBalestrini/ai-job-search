...
version: 1.0.0
description: >
  Generate and verify a new job portal search skill for a local job board. Trigger when the user wants to add a job board, portal, scraper, or search integration.
context: fork
---

# add-portal workflow - Generate a Job-Portal Search Skill for Your Local Market
...
`the user request` may contain a subcommand, a portal URL, or nothing.
...
- If `the user request` contains `--list`: use Glob with `.agents/skills/*/SKILL.md`, print a table of installed portal skills (name, market from the description, data source from `url-reference.md`), and stop.
- If `the user request` contains a URL: treat it as the portal URL and carry it into Step 1.
...
3. **Market and language** - which country/region the portal covers and what language its postings use. This drives the trigger phrases in `SKILL.md` (include local-language terms like the Danish skills do: "ledige stillinger", "jobs?gning").
...
Do reconnaissance before writing any code. Use web fetch (or `curl` via the shell) on the portal:
...
These conventions are what make portal skills interchangeable for `the job-scraper skill` and for users reading any skill's docs:
...
- **Fetching:** browser User-subagent, exponential backoff with jitter on 429/5xx (max ~6 retries), `""`/`null` on 404 rather than a crash.
...
1. Ask whether the user wants the new portal added to their `the job-scraper skill` search strategy. If yes, add the portal's site to the relevant query categories in `.agents/skills/job-scraper/search-queries.md` (site-specific queries, like the existing `jobindex.dk` entries) so `the job-scraper skill` includes it.
