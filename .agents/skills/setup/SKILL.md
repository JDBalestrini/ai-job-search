...
If `the user request` contains `--section <name>`, skip directly to that section in Path C for an update-only flow. Do not run the path-selection prompt below.
...
Apply the confirmed changes with the targeted file-editing capability. Make targeted edits only. Do not rewrite entire files. State which changes were applied per file. If a file has no confirmed changes, state "No changes made to [filename]."

Documents cover skills, experience, education, references, and behavioral signal. They do not cover everything `the apply skill` and `the job-scraper skill` need. After the writes, ask follow-up questions for gaps:
...
This section generates the search queries that power `the job-scraper skill`. Use the information from Sections 1, 4, and 7 to build targeted queries.
...
> - Run `the setup skill --section search` later to update your search queries as your priorities evolve
...
- Can be re-run with `--section <name>` to update specific sections (e.g., `the setup skill --section search` to reconfigure job search queries without re-doing the full profile).
...
- At the end, suggest running `the job-scraper skill` and `the apply skill` with a test job posting.
