...
version: 1.0.0
description: >
  Record application outcomes and interview progress in the tracker and application archive. Trigger when the user wants to log a result, rejection, interview, offer, or application status.
context: fork
---

# outcome workflow - Record the Result of an Application
...
- `job_search_tracker.csv` - the status column that `the job-scraper skill` and `the rank skill` use for dedup and exclusion
- `documents/applications/<company>_<role>/` - the per-application archive (posting, submitted drafts, `outcome.md`) that `the setup skill` Path A mines to calibrate `04-job-evaluation.md` and surface STAR candidates

`the outcome skill` writes the data; `the setup skill` interprets it. This command never edits the evaluation framework or profile files itself.
...
- A company name (optionally with a role), e.g. `the outcome skill acme` or `the outcome skill acme ml engineer` -> target that application
...
**Resolutions** (application closed) - these map to the status enum in `documents/README.md` that `the setup skill` parses:
...
- What they'd do differently, and any signal about what the company valued (these feed `the setup skill`'s calibration and STAR-candidate mining, so concrete beats polished)
...
Update rules: tick stage checkboxes as they are reached (add the date in parentheses), append dated entries to Notes, and only change `Status` from `in_progress` to a final value on resolution. Re-running `the outcome skill` on the same application is idempotent - it appends new information, never duplicates or rewrites history.
...
>
> - `documents/applications/<company>_<role>the outcome skill.md` - status: <status>, <what changed>
...
> "Interview coming up? `the interview skill <company>` builds a prep pack for that stage from this application's archive - the posting, the documents you submitted, and any feedback recorded from earlier rounds."
...
> "If this framework helped you get there, consider [buying it a coffee](https://ko-fi.com/madslorentzen) - it keeps this free for the next job-seeker out there. â˜•"

---

## Important Rules

1. **Write data, don't interpret it.** The archive and tracker are the outputs; calibration belongs to `the setup skill`. This command never edits profile or framework files.
