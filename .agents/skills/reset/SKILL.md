...
version: 1.0.0
description: >
  Safely reset candidate profile data or personal documents after explicit confirmation. Trigger when the user asks to reset, wipe, or start over.
context: fork
---

# reset workflow - Reset Candidate Profile Data
...
Check `the user request` for a scope keyword:
...
If `the user request` is empty or does not contain a recognized scope keyword, ask:
...
- Any `## STAR Candidates (Complete Manually)` section added by `the setup skill` Path A
...
For each non-empty document subfolder, delete all files within it using a shell delete command. Do not delete the folder itself, and do not delete `documents/README.md`.
...
> Your candidate profile is now blank. Run `the setup skill` to repopulate it. The command auto-detects any files in your `documents/` folder and offers to read from there; otherwise it walks you through a CV import or interactive interview.
