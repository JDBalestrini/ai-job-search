...
version: 1.0.0
description: >
  Register, verify, and activate a custom LaTeX CV or cover-letter template. Trigger when the user wants to add, list, switch, or reset application document templates.
context: fork
---

# add-template workflow - Register a Custom CV or Cover Letter Template

You are helping the user register their own LaTeX template with the AI Job Search framework. The framework ships with moderncv (banking style) for CVs and a custom `cover.cls` for cover letters. This command lets the user swap in their own template: store the template files, capture usage instructions (compile engine, fonts, style rules, page limits), verify the template compiles, and wire it into the `the apply skill` workflow so every future application uses it.

`the user request` may contain a subcommand, a file path, or nothing.
...
- If `the user request` contains `--list`: run **List Mode** below and stop.
- If `the user request` contains `--use <name>`: run **Switch Mode** below, then continue to **Step 5: Activate** with the resolved template metadata. `--use default` deactivates any custom template and restores the stock guidance (see Step 5).
- If `the user request` contains a file path or @-mentioned file: treat it as the template source and carry it into Step 1.
...
A template is **active** if `05-cv-templates.md` (CV) or `06-cover-letter-templates.md` (cover letter) contains an `ACTIVE-TEMPLATE` managed block naming it. If no custom templates exist, say so and explain that `the add-template skill` registers one. Stop here.

### Switch Mode

If `the user request` contains `--use <name>`:
...
3. If no manifest matches, stop and say the template is not registered. Suggest `the add-template skill --list` to see available names.
...
Ask the user (skip anything already answered by `the user request`):
...
Interview the user for the metadata that `the apply skill` needs to use the template correctly. Infer as much as possible from the LaTeX source first (documentclass, `\fontspec` calls, geometry, colors) and present your inferences for confirmation rather than asking blind questions.
...
4. On success, Inspect the PDF and confirm the layout renders sensibly (no overlapping text, fonts loaded, page count plausible for dummy content). Record any surprises in the manifest's "Known pitfalls".
...
Activation wires the template into `the apply skill` by adding a **managed block** to the top of the relevant guidance file - `05-cv-templates.md` for CVs, `06-cover-letter-templates.md` for cover letters. `the apply skill` reads these files in its drafting step, so the block is all it takes.
...
```markdown
<!-- BEGIN ACTIVE-TEMPLATE (managed by the add-template skill - do not edit by hand) -->
...
>
> Useful follow-ups:
> - `the add-template skill --list` - see all registered templates
> - `the add-template skill --use <other-name>` - switch templates
> - `the add-template skill --use default` - go back to the stock <moderncv | cover.cls> template
...
- The compile check in Step 4 is non-negotiable - a template that has never compiled will fail mid-`the apply skill`, which is the worst place to discover it.
- Activation is a small managed block, not a rewrite of the guidance files: `the setup skill` and manual edits to `05`/`06` survive template switches, and `--use default` is a clean revert.
