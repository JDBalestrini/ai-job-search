...
version: 1.0.0
description: >
  Enrich the candidate profile with source-traced competencies from documents and public online presence. Trigger on profile expansion, skill discovery, or competency enrichment.
context: fork
---

# expand workflow - Competency Expansion from Documents and Online Presence
...
1. Use web fetch or web search to retrieve the public profile and pinned repositories
...
```
## the expand skill found [N] new competency signals across [M] sources
...
Apply only the confirmed items. Use the targeted file-editing capability to add to the relevant sections of each file - do not rewrite entire files.
...
For each addition, add a brief source annotation in a comment or parenthetical: *(Coursera - Deep Learning Specialisation)*, *(GitHub - project-name)*, etc. This makes future `the expand skill` runs idempotent.
